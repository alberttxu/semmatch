def main():
    ...
    parser.add_argument('--newLabel', help='starting label of added points', required=True)
    parser.add_argument('-o', '--output', help='output nav file', default='semmatchNav.nav', required=True)
    ...
    parser.add_argument('--groupRadius', help='groupRadius in µm', required='--gui' not in sys.argv)
    parser.add_argument('--pixelSize', help='pixelSize in nm', required='--gui' not in sys.argv)
    ...
    if args.groupOption != '1' and (args.groupRadius is not None or args.pixelSize is not None):
        parser.error("groupRadius and pixelSize only valid for groupOption 1: groups within mesh")
